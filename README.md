...
### 4. MERA Language Model
MERA (Multi-scale Entanglement Renormalization Ansatz) に着想を得た階層的モデル。U-Net型のエンコーダ・デコーダ構造で、各スケールで因果的disentanglerとisometryを適用し、最粗スケールでMPS再帰を行う。

```
Token + Position Embedding → MERABlock (Causal Disentangle → Pool → ... → MPS → ... → Un-pool → Disentangle + Skip) → LayerNorm → LM Head
```
...
WikiText-2, vocab=8192, d_model=256, 5 epochs:

| Metric | MPS | Transformer | Hybrid | MERA |
|---|---|---|---|---|
| Parameters | 4.25M | 4.24M | 4.25M | 4.24M |
| **Test Perplexity** | **677.8** | 820.0 | 696.8 | 793.3 |
| Inference (tok/s) | 545K | **1,636K** | 567K | 754K |
| Inference (ms/batch) | 7.5 | **2.5** | 7.2 | 5.4 |
| Peak Memory (MB) | **222.1** | 287.0 | 286.9 | 286.9 |
| Train throughput (step/s) | 42.7 | **113.5** | 43.5 | 41.8 |
| Training time (5 epochs) | 43s | **17s** | 42s | 30s |

### パラメータ効率の比較

MPS はより少ないパラメータで Transformer を上回る:

| Model | Parameters | Test PPL |
|---|---|---|
| **MPS-small** | **3.13M** | **764.3** |
| TF-small | 2.92M | 936.4 |
| **MPS-medium** | **3.67M** | **715.7** |
| TF-medium | 3.51M | 886.6 |
| **MPS-full** | **4.25M** | **677.8** |
| TF-full | 4.24M | 820.0 |

**MPS 3.13M パラメータ (PPL 764) が Transformer 4.24M パラメータ (PPL 820) を上回る** — 26% 少ないパラメータでより高い性能を達成。

- **MPS が全サイズで最良の Perplexity** — Transformer より 17-22% 低い
...
│   │   ├── mera_layer.py    # MERA: hierarchical U-Net with causal isometries
│   │   ├── mera_lm.py       # Full MERA Language Model
...
├── scripts/
│   ├── compare.py            # Train & compare all models (MPS, Transformer, Hybrid, MERA)
...
# Train & compare all four models (MPS, Transformer, Hybrid, MERA)
